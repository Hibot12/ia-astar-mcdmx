...
class CoordinatePicker:
    def __init__(self):
        ...
        # Mostrar dimensiones
        print(
            f"Dimensiones de la imagen: {self.original_img.width}x{self.original_img.height}"
        )

        # Crear canvas
        self.canvas = tk.Canvas(
            self.root, width=self.original_img.width, height=self.original_img.height
        )
        self.canvas.pack()
        ...
        self.label = tk.Label(
            self.root, text="Haz clic en cada estación", font=("Arial", 12)
        )
        self.label.pack()
        ...
        self.save_btn = tk.Button(
            self.root, text="Guardar Coordenada", command=self.save_coord
        )
        self.save_btn.pack()
...
5. Al cerrar, se imprimirán todas las coordenadas
        ...
        self.instructions = tk.Label(
            self.root, text=instructions, justify=tk.LEFT, font=("Arial", 9)
        )
